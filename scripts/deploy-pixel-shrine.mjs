...
const source = await readFile(resolve(process.cwd(), "contracts/PixelShrine.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "PixelShrine.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["PixelShrine.sol"]?.PixelShrine;
...
console.log(`Deploying PixelShrine from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_PIXEL_SHRINE_CONTRACT_ADDRESS to this contract address in Vercel.");
